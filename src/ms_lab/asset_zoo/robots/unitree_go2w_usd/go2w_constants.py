...
GO2W_USD: Path = (
  ms_lab_SRC_PATH / "asset_zoo" / "robots" / "unitree_go2w_usd" / "usds" / "go2w_description.usd"
)
assert GO2W_USD.exists()
...
GO2W_HIP_ACTUATOR_CFG = ActuatorCfg(
...
GO2W_KNEE_ACTUATOR_CFG = ActuatorCfg(
...
GO2W_FOOT_ACTUATOR_CFG = ActuatorCfg(
...
GO2_ARTICULATION = EntityArticulationInfoCfg(
  actuators=(
    GO2W_HIP_ACTUATOR_CFG,
    GO2W_KNEE_ACTUATOR_CFG,
    GO2W_FOOT_ACTUATOR_CFG,
  ),
  soft_joint_pos_limit_factor=0.9,
)

GO2W_ROBOT_CFG = EntityCfg(
  init_state=INIT_STATE,
  collisions=(FULL_COLLISION,),
  spec_fn=None ,
  asset_file=str(GO2W_USD),
  articulation=GO2_ARTICULATION,
)

GO2W_ACTION_SCALE: dict[str, float] = {}
for a in GO2_ARTICULATION.actuators:
  ...
  for n in names:
    if n in e and n in s and s[n]:
      GO2W_ACTION_SCALE[n] = 0.25 * e[n] / s[n]
