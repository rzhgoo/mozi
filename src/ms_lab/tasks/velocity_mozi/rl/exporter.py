...
from ms_lab.envs.mdp.actions.joint_actions import JointAction
from ms_lab.third_party.isaaclab.isaaclab_rl.rsl_rl.exporter import _OnnxPolicyExporter
...
def attach_onnx_metadata(
  ...
  robot: Entity = env._backend.get_robot("robot")
  onnx_path = os.path.join(path, filename)
  joint_action = env.action_manager.get_term("joint_pos")
  assert isinstance(joint_action, JointAction)
  ctrl_ids = robot.indexing.ctrl_ids.cpu().numpy()
  joint_stiffness = env._backend.get_stiffness(ctrl_ids)
  joint_damping = -env._backend.get_dampling(ctrl_ids)
  metadata = {
    "run_path": run_path,
